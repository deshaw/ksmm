...
interface IKSMMAdditonalFields {
  name: string;
  deletable: boolean;
  writeable: boolean;
  fs_path: string;
...
const KernelManagerComponent = ({
    ...
    const dialog = new Dialog({
      ...
      body: ReactWidget.create(
        <>
          ...
          >
            <i>
              *We are not validating the form yet, please ensure you enter valid
              data (
              <a href="https://github.com/deshaw/ksmm/issues/61" target="blank">
                issue
              </a>
              )
            </i>
        ...
        requestAPI('/params', {
          ...
          .catch(err => {
            return showErrorMessage(
              'Could not generate kernel from template',
              err,
            );
          })
          .then((_data: any) => {
  ...
  const handleSubmitKernelspec = (e: any) => {
    ...
    /* eslint @typescript-eslint/no-unused-vars: off -- There's legitimately no cleaner way to do this */
  ...
  const KernelSpecCard = ({
    kernelSpec,
  }: {
    kernelSpec: EnhancedKernelSpec;
  }) => (
    <KsCard
  ...
  const sortByDisplayName = (
    k1: EnhancedKernelSpec,
    k2: EnhancedKernelSpec,
  ) => {
  ...
  const userKernelCards = Object.values(userKernels)
    .sort(sortByDisplayName)
    .map(kernelSpec => <KernelSpecCard kernelSpec={kernelSpec} />);
  const systemKernelCards = Object.values(systemKernels)
    .sort(sortByDisplayName)
    .map(kernelSpec => <KernelSpecCard kernelSpec={kernelSpec} />);
  return (
    <Container>
      ...
      {!showForm && (
        <>
          {systemKernelCards.length > 0 ? (
            <>
              <h3 className="h3">System Kernel Specs</h3>
              <div className="ksmm-card-container">{systemKernelCards}</div>
            </>
          ) : null}
          ...
          {userKernelCards.length > 0 ? (
            <>
              <h3 className="h3">User Kernel Specs</h3>
              <div className="ksmm-card-container">{userKernelCards}</div>
            </>
          ) : null}
        </>
      )}

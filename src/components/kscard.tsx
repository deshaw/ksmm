...
const KsCard = (props: any): JSX.Element => {
  ...
  const renderToolTip = (props: any): JSX.Element => (
    <Tooltip id="card-tooltip" {...props}>
      {kernelSpec._ksmm.fs_path}
    </Tooltip>
  );

  return (
    <Card className="ksmm-kernel-cards" key={kernelSpec._ksmm.fs_path}>
      <Card.Header className="align-left">
        {kernelSpec._ksmm?.writeable ? (
          <a
